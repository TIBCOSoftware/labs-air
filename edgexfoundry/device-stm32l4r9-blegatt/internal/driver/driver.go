...
func onPeriphDiscovered(p gatt.Peripheral, a *gatt.Advertisement, rssi int) {
	...
	fmt.Println("In onPeriphDiscovered")
	id := "ab53b5b16f8f4f8889b2cb899595751c"
	fmt.Println("ARG: ", id)
	fmt.Println("FND: ", strings.ToUpper(p.ID()))
	fmt.Println("LOC: ", strings.ToUpper(a.LocalName))
	fmt.Println("SVC: ", a.Services)

	// for _, svc := range a.Services {
	// 	fmt.Println("checking svc: ", svc.String())
	// 	if svc.String() == id {
	// 		fmt.Println("Found service ", id)
	// 		found = true
	// 		continue
	// 	}
	// }
