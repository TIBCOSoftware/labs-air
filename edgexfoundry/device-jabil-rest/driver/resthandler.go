...
func (handler RestHandler) processAsyncRequest(writer http.ResponseWriter, request *http.Request) {
	...
	handler.lastMsg.Temperature = inMsg.Temperature
	handler.lastMsg.Humidity = inMsg.Humidity
	handler.lastMsg.Lux = inMsg.Lux
	handler.lastMsg.Motion = inMsg.Motion

	resourceNames := []string{"Temperature", "Humidity", "Lux", "Motion"}
	...
	for i, rn := range resourceNames {
		...
		if rn == "Temperature" {
			value, err = handler.newCommandValue(commandRequest, inMsg.Temperature, timestamp)
		} else if rn == "Humidity" {
			value, err = handler.newCommandValue(commandRequest, inMsg.Humidity, timestamp)
		} else if rn == "Lux" {
			value, err = handler.newCommandValue(commandRequest, inMsg.Lux, timestamp)
		} else if rn == "Motion" {
			value, err = handler.newCommandValue(commandRequest, inMsg.Motion, timestamp)
		}
