	...
	for i, req := range reqs {
		...
		if req.DeviceResourceName == "Temperature" {
			if req.Type == dsModels.Float32 {
				reading = fmt.Sprintf("%f", driver.restHandler.lastMsg.Temperature)
			} else if req.Type == dsModels.Int16 {
				reading = fmt.Sprintf("%d", driver.restHandler.lastMsg.Temperature)
			} else if req.Type == dsModels.Bool {
				reading = fmt.Sprintf("%t", driver.restHandler.lastMsg.Temperature)
			}
		} else if req.DeviceResourceName == "Humidity" {
			if req.Type == dsModels.Float32 {
				reading = fmt.Sprintf("%f", driver.restHandler.lastMsg.Humidity)
			} else if req.Type == dsModels.Int16 {
				reading = fmt.Sprintf("%d", driver.restHandler.lastMsg.Humidity)
			} else if req.Type == dsModels.Bool {
				reading = fmt.Sprintf("%t", driver.restHandler.lastMsg.Humidity)
			}
		} else if req.DeviceResourceName == "Lux" {
			if req.Type == dsModels.Float32 {
				reading = fmt.Sprintf("%f", driver.restHandler.lastMsg.Lux)
			} else if req.Type == dsModels.Int16 {
				reading = fmt.Sprintf("%d", driver.restHandler.lastMsg.Lux)
			} else if req.Type == dsModels.Bool {
				reading = fmt.Sprintf("%t", driver.restHandler.lastMsg.Lux)
			}
		} else if req.DeviceResourceName == "Motion" {
			if req.Type == dsModels.Float32 {
				reading = fmt.Sprintf("%f", driver.restHandler.lastMsg.Motion)
			} else if req.Type == dsModels.Int16 {
				reading = fmt.Sprintf("%d", driver.restHandler.lastMsg.Motion)
			} else if req.Type == dsModels.Bool {
				reading = fmt.Sprintf("%t", driver.restHandler.lastMsg.Motion)
			}
		}
