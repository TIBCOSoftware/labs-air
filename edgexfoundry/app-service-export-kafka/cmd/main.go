...
import (
	...
	inttransforms "github.com/TIBCOSoftware/labs-air/edgexfoundry/app-service-export-kafka/internal/transforms"
...
const (
	serviceKey = "app-service-export-kafka"
)
...
var kafkaSender *inttransforms.KafkaSender
...
func main() {
	...
	// Create the Kafka Sender
	kafkaConfig, _ := inttransforms.LoadKafkaConfig(edgexSdk)
	kafkaSender = inttransforms.NewKafkaSender(edgexSdk.LoggingClient, kafkaConfig)
...
func processEvent(edgexcontext *appcontext.Context, params ...interface{}) (bool, interface{}) {
	...
	if event, ok := params[0].(models.Event); ok {
		...
		// Export event
		return kafkaSender.KafkaSend(string(encjson))
