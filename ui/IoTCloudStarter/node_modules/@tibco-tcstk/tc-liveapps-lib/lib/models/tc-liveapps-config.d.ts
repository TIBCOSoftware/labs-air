import { Deserializable, GeneralConfig } from '@tibco-tcstk/tc-core-lib';
export declare class LiveAppsConfig implements Deserializable {
    id: string;
    applicationIds: string[];
    recentExcludedAppIds: string[];
    caseIconsFolderId: string;
    documentAppId: string;
    collaborationAppId: string;
    deserialize(input: any): this;
}
export declare class LiveAppsConfigHolder implements Deserializable {
    generalConfig: GeneralConfig;
    liveAppsConfig: LiveAppsConfig;
    deserialize(input: any): this;
}
